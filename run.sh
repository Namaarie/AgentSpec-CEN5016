...
require_openai_api_key() {
	if [ -z "${OPENAI_API_KEY:-}" ]; then
		echo "OPENAI_API_KEY is required for the LangChain smoke demo."
		echo "Set it in your shell environment or .env file, then retry."
		exit 1
	fi
}
...
run_test_8() {
	...
	run_in_root java -jar "$ANTLR_JAR" -Dlanguage=Python3 ./src/spec_lang/AgentSpec.g4
